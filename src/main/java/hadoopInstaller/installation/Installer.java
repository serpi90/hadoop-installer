...
public class Installer {
	public static final String INSTALLER_NAME = Messages.getString("HadoopInstaller.InstallerName"); //$NON-NLS-1$
	...
	private void generateConfigurationFiles() throws InstallationFatalError {
		...
		this.configurationFilesToUpload = new LoadFromFolder(InstallerConstants.CONFIGURATION_FOLDER_TO_UPLOAD,
				this.getLocalDirectory(), this.getLog()).generateConfigurationFiles();
	...
	public void getConfigurationFilesToUpload(FileObject someConfigurationFilesToUpload) {
	...
	private FileObject getConfigurationSchema() throws InstallationFatalError {
		...
		try {
			...
			try (OutputStream out = configurationSchema.getContent().getOutputStream();
					InputStream in = this.getClass().getResourceAsStream(InstallerConstants.CONFIGURATION_SCHEMA);) {
	...
	public void run() throws InstallationFatalError {
		...
		for (Host host : getConfig().getNodes()) {
			try {
				...
				if (e.getCause() != null) {
					getLog().error(e.getCause(), "HadoopInstaller.Installation.HostFailed", //$NON-NLS-1$
							host.getHostname());
				} else {
